...
<script type="text/javascript" src="/minisjksjpjpfshop/Public/plugins/xheditor/xheditor-1.2.1.min.js"></script>
<script type="text/javascript" src="/minisjksjpjpfshop/Public/plugins/xheditor/xheditor_lang/zh-cn.js"></script>
<script type="text/javascript" src="/minisjksjpjpfshop/Public/ht/js/jCalendar.js"></script>
<style>
      ...
      <li>
        ...
        <div>
         ...
         <li>
            ...
            <div>
              <select class="inp_1" name="cateid" id="cateid" onchange="getcatid();" style="width:150px;margin-right:5px;">
                <!-- 遍历 -->
                <option value="">一级分类</option>
                <?php if(is_array($cate_list)): $i = 0; $__LIST__ = $cate_list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$v): $mod = ($i % 2 );++$i;?><option value="<?php echo ($v["id"]); ?>" <?php if($v["id"] == $pro_allinfo['cateid']): ?>selected="selected"<?php endif; ?>>-- <?php echo ($v["name"]); ?></option><?php endforeach; endif; else: echo "" ;endif; ?>
                <!-- 遍历 -->
              </select>
              <select class="inp_1" name="catid" id="catid" onchange="getcid();" style="width:150px;margin-right:5px;">
                <option value="">二级分类</option>
                <?php if ($catetwo) { ?>
                  <?php if(is_array($catetwo)): $i = 0; $__LIST__ = $catetwo;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$ct): $mod = ($i % 2 );++$i;?><option value="<?php echo ($ct["id"]); ?>" <?php if($ct["id"] == $pro_allinfo['catid']): ?>selected="selected"<?php endif; ?>>-- <?php echo ($ct["name"]); ?></option><?php endforeach; endif; else: echo "" ;endif; ?>
                <?php } ?>
              </select>
              <select class="inp_1" name="cid" id="cid" style="width:150px;">
                <option value="">三级分类</option>
                <?php if ($catethree) { ?>
                  <?php if(is_array($catethree)): $i = 0; $__LIST__ = $catethree;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$cs): $mod = ($i % 2 );++$i;?><option value="<?php echo ($cs["id"]); ?>" <?php if($cs["id"] == $pro_allinfo['cid']): ?>selected="selected"<?php endif; ?>>-- <?php echo ($cs["name"]); ?></option><?php endforeach; endif; else: echo "" ;endif; ?>
                <?php } ?>
              </select>
              <span id="catedesc" style="color:red;font-size: 12px;">&nbsp;&nbsp; * 一级和二级分类为必选项，三级为非必选</span>
            </div>
         </li>
         <!-- 产品分类 -->
          ...
          <div class="d1">缩略图:</div>
           <div>
            <?php if ($pro_allinfo['photo_x']) { ?>
                  <img src="/minisjksjpjpfshop/Data/<?php echo $pro_allinfo['photo_x']; ?>" width="80" height="80" style="margin-bottom: 3px;" />
                  <br />
              <?php } ?>
          ...
          <div class="d1">大 图:</div>
           <div>
            <?php if ($pro_allinfo['photo_d']) { ?>
                  <img src="/minisjksjpjpfshop/Data/<?php echo $pro_allinfo['photo_d']; ?>" width="125" height="125" style="margin-bottom: 3px;" />
                <br />
              <?php } ?>
          ...
          <?php foreach ($img_str as $v) { ?>
           <div>
            <div class="img-err" title="删除" onclick="del_img('<?php echo $v; ?>',this);">×</div>
             ...
             <?php }else{ ?>
              <img src="<?php echo '/minisjksjpjpfshop/Data/'.$v; ?>" width="125" height="125">
             <?php } ?>
           </div>
          <?php } ?>
         </li>
        ...
        <li>
         ...
         <li>
            <div class="d1">商品类型:</div>
            <div>
              <select class="inp_1" name="pro_type" id="pro_type" onchange="showli();" style="width:150px;margin-right:5px;">
                <option value="1" <?php if (intval($pro_allinfo['pro_type'])<2) { ?>selected="selected"<?php } ?>>普通商品</option>
                <option value="2" <?php if($pro_allinfo["pro_type"] == 2): ?>selected="selected"<?php endif; ?>>团购预售商品</option>
                <option value="3" <?php if($pro_allinfo["pro_type"] == 3): ?>selected="selected"<?php endif; ?>>采购商品</option>
              </select>
            </div>
         </li>
         <li id="yu2"  <?php if (intval($pro_allinfo['pro_type'])==2) { ?>style="display:block;"<?php }else{ ?>style="display:none;"<?php } ?>>
            <div class="d1">起团数:</div>
            <div>
              <input class="inp_1 inp_6" name="yu_num" id="yu_num" value="<?php echo ($pro_allinfo["yu_num"]); ?>"/>
              请根据库存合理设置起团数量
            </div>
         </li>
         <li id="yu" <?php if (intval($pro_allinfo['pro_type'])==2) { ?>style="display:block;"<?php }else{ ?>style="display:none;"<?php } ?>>
            <div class="d1">预售单价:</div>
            <div>
              <input class="inp_1 inp_6" name="yu_price" id="yu_price" value="<?php echo ($pro_allinfo["yu_price"]); ?>"/>
            </div>
         </li>
         <li id="cai"  <?php if (intval($pro_allinfo['pro_type'])==3) { ?>style="display:block;"<?php }else{ ?>style="display:none;"<?php } ?>>
            <div class="d1">最低采购数:</div>
            <div>
              <input class="inp_1 inp_6" name="pifa_num" id="pifa_num" value="<?php echo ($pro_allinfo["pifa_num"]); ?>"/>
              请根据库存合理设置最低采购数量
            </div>
         </li>
         <li id="cai2"  <?php if (intval($pro_allinfo['pro_type'])==3) { ?>style="display:block;"<?php }else{ ?>style="display:none;"<?php } ?>>
            <div class="d1">采购单价:</div>
            <div>
              <input class="inp_1 inp_6" name="pifa_price" id="pifa_price" value="<?php echo ($pro_allinfo["pifa_price"]); ?>"/>
            </div>
         </li>
...
<script type="text/javascript" src="/minisjksjpjpfshop/Public/ht/js/product.js"></script>
<script>
function upadd(obj){
  //alert('aaa');
...
function getcatid(){
  ...
  $.post('<?php echo U("getcid");?>',{cateid:cateid},function(data){
      if(data.catelist!=''){
        ...
        $('#catid').html(htmls);
        $('#catedesc').html('&nbsp;&nbsp; * 一级和二级分类为必选项，三级为非必选');
      }else{
        $('#catid').html('<option value="">二级分类</option>');
...
function getcid(){
  var catid = $('#catid').val();
  $.post('<?php echo U("getcid");?>',{cateid:catid},function(data){
      if(data.catelist!=''){
        var htmls = '<option value="">三级分类</option>';
        var cate = data.catelist;
        for (var i = 0; i<cate.length; i++) {
          htmls += '<option value="'+cate[i].id+'">-- '+cate[i].name+'</option>';
        }
...
function ac_from(){
  ...
  var cid=parseInt(document.getElementById("catid").value);
  if(!cid){
    alert("请选择分类.");
    return false;
  }
...
function showli(){
  ...
  if (pro_type==2) {
    ...
    $('#yu2').css('display','block');
    $('#cai').css('display','none');
    $('#cai2').css('display','none');
  }else if (pro_type==3) {
    //批发商品
    $('#yu').css('display','none');
    $('#yu2').css('display','none');
    $('#cai').css('display','block');
    $('#cai2').css('display','block');
  }else if (pro_type==4) {
    //体验商品
    $('#yu').css('display','none');
    $('#yu2').css('display','none');
