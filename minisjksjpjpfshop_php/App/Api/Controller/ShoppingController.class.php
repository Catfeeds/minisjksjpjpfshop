...
class ShoppingController extends PublicController {
	...
	public function index(){
		...
		$cart = $shopping->where('uid='.intval($user_id))->field('id,uid,pid,price,num,buff,type')->select();
        foreach ($cart as $k => $v) {
	...
	public function add(){
		...
		$shu = $_POST['darr'];
		if($shu && intval($check_info['is_buff'])>0){
			//判断用户是否已经选择完规格，未选完则提示错误
			$shu = json_decode($shu,true);
			$carts = '';
			foreach ($shu as $k => $v) {
				if (intval($v['num'])>0) {
					$da = array();
					...
					$da['addtime']=time();
					$da['uid']=intval($uid);
					$da['shop_id']=intval($check_info['shop_id']);
					$ptype = 1;
					if (intval($check_info['pro_type'])) {
						$ptype = intval($check_info['pro_type']);
					}
					$da['buff'] = str_replace("-",",",$v['keys']);
					$da['type']=$ptype;
					$da['price'] = floatval($v['price']);
					$re = $shpp->add($da);
					$carts .= $re.',';
				}
			}
			echo json_encode(array('status'=>1,'cart_id'=>$carts)); //该商品已成功加入您的购物车
			exit;
		}
