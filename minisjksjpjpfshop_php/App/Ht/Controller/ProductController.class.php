...
class ProductController extends PublicController{
	...
	public function add(){	
		...
		try{	
			...
			$array=array(
				...
				'cateid'=> intval($_POST['cateid']) ,			//产品一级分类ID
				'catid'=> intval($_POST['catid']) ,			//产品二级分类ID
				'cid'=> intval($_POST['cid']) ,			//产品三级分类ID
				...
				'pro_type'=>intval($_POST['pro_type']),//产品类型
				'renqi' => intval($_POST['renqi']),
				'is_hot'=>intval($_POST['is_hot']),//是否热卖
				'is_show'=>intval($_POST['is_show']),//是否新品
				'yu_price'=>(float)$_POST['yu_price'],//团购预售单价
				'yu_num' =>intval($_POST['yu_num']), //起团数量
				'pifa_num'=>intval($_POST['pifa_num']),//最低采购数量
				'pifa_price'=>(float)$_POST['pifa_price'],//批发单价
			);

			$pro_type = intval($_POST['pro_type']);
			if ($pro_type==2) {
				$array['price_yh'] = (float)$_POST['yu_price'];
			}elseif ($pro_type==3) {
				$array['price_yh'] = (float)$_POST['pifa_price'];
			}elseif ($pro_type==4) {
				$array['price_yh'] = (float)$_POST['ti_price'];
			}
		...
		//产品分类
		if (intval($pro_allinfo['cateid'])) {
			$catetwo = M('category')->where('tid='.intval($pro_allinfo['cateid']))->field('id,name')->select();
			$this->assign('catetwo',$catetwo);
		}
		if (intval($pro_allinfo['catid'])) {
			$catethree = M('category')->where('tid='.intval($pro_allinfo['catid']))->field('id,name')->select();
			$this->assign('catethree',$catethree);
		}
	...
	*/
	...
	//产品推荐管理首页
	public function pro_tj(){
		$tj_list = M('pro_tj')->select();
		$this->assign('tj_list',$tj_list);
		$this->display();
	}

	//添加、修改产品推荐栏目
	public function add_tj(){
		$id = intval($_GET['id']);
		$tj = M('pro_tj')->where('id="'.$id.'"')->find();
		$this->assign('tj',$tj);
		$this->display();
	}
	...
	public function save_tj(){
		$id = intval($_POST['id']);
		$data['pid'] = $_POST['proid'];
		$data['name'] = $_POST['name'];
		if($id){
			$res = M('pro_tj')->where('id="'.$id.'"')->save($data);
		}else{
			$res = M('pro_tj')->add($data);
		}
		if($res){
			$this->success('操作成功！','pro_tj');
			exit();
		}else{
			$this->error();
			exit();
	...
	public function tj_del(){
		$id = intval($_GET['id']);
		$res = M('pro_tj')->where('id="'.$id.'"')->delete();
		if($res){
			$this->success('操作成功！','pro_tj');
			exit();
		}else{
	...
	public function tj_show(){
		$id = intval($_GET['id']);
		$type = M('pro_tj')->where('id="'.$id.'"')->getField('type');
		if($type == 0){
			$tmp['type'] = 1;
			M('pro_tj')->where('id="'.$id.'"')->save($tmp);
		}else{
			$tmp['type'] = 0;
			M('pro_tj')->where('id="'.$id.'"')->save($tmp);
		}
