	...
	public function _initialize(){
		...
		foreach ($list as $k1 => $v1) {
			...
			foreach ($list[$k1]['list2'] as $k2 => $v2) {
				...
				foreach ($list[$k1]['list2'][$k2]['list3'] as $k3 => $v3) {
					$list[$k1]['list2'][$k2]['list3'][$k3]['list4'] = $this->category->where('tid='.intval($v3['id']))->order('sort desc,id asc')->field('id,tid,name,bz_2,sort')->select();
				}
			}
		}
	...
	*/
	public function save(){
		...
		if (intval($tid1)) {
			...
			if (intval($tid2)) {
				$tid3 = $this->category->where('id='.intval($tid2))->getField('tid');
				if (intval($tid3)) {
					$c_info = $this->category->where('id='.intval($tid3))->find();
					if ($c_info) {
